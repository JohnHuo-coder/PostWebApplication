...
import cover from "../assets/marco-grosso-0OxGTvdxL-U-unsplash.jpg"
...
export default function MyProfile() {
	...
	useEffect(() => {
		...
		const fetchUserPosts = async() => {
			try{
				const res = await fetch(`${API_URL}/api/myprofile/posts`, {
					method: "GET",
					credentials: "include"
				})
				const data = await res.json();
				if (!res.ok){
					console.log("error fetching user posts, may due to internet error or not authenticated", data.error)
					setError(data.error || 'fail fetching user');
					setErrorStatus(res.status)
					if (errorStatus === 401){
							refreshAuth();
	...
	const deletePost = async(postId) => {
		...
		try{
			const res = await fetch(`${API_URL}/api/myprofile/posts/${postId}/delete`, {
				method: "DELETE",
				credentials: "include"
			})
			const data = await res.json();
			...
			<div className="bg-gray-900 py-16 sm:py-16 w-full">
						...
						<div className="-mt-12 sm:-mt-16 sm:flex sm:items-end sm:space-x-5" >
							...
							<div className="mt-6 sm:flex sm:min-w-0 sm:flex-1 sm:items-center sm:justify-end sm:space-x-6 sm:pt-10">
								...
								<div className="mt-6 flex flex-col justify-stretch space-y-3 sm:flex-row sm:space-y-0 sm:space-x-4">
									<Link to="/myprofile/edit/public" className="inline-flex items-center justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-xs inset-ring inset-ring-gray-300 hover:bg-gray-50 dark:bg-white/10 dark:text-white dark:shadow-none dark:inset-ring-white/5 dark:hover:bg-white/20">
				...
				<div className="px-4 py-5 sm:px-5 sm:py-5">
					...
					<div className="sm:flex sm:px-6 sm:py-5">
						...
						<dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0 sm:ml-6 dark:text-white">
						{userProfile.bio ? <p>{userProfile.bio}</p> : <p className="text-gray-500">Share something about yourself</p>}
						</dd>
					</div>
					</dl>
				</div>
				...
				<div className="mx-auto max-w-8xl px-6 py-6 sm:py-8 lg:px-8" >
					...
					<h3 className="text-4xl font-semibold tracking-tight text-pretty text-white sm:text-4xl">Past posts</h3>
					<p className="mt-4 text-lg/8 text-gray-300">Take a look of your accomplishments.</p>
										...
										<article className="flex max-w-xl flex-col items-start justify-between">
											<div className="mb-3 flex items-center gap-x-4 text-xs">
												...
												<a  href="#"
													...
													>
													{post.show_info ? "public" : "anonymous"}
												</a>
