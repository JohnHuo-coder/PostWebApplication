...
export default function PrivateForm() {
    ...
    const skipPrivate = async () => {
        try{
            const res = await fetch(`${API_URL}/api/onboard/skip-private`, {
                method: "POST",
                credentials: "include"
            });
            const data = res.json()
            ...
            console.log("complete private profile successfully", data);
            refreshAuth();
        }catch(e){
    ...
    try{
        const res = await fetch(`${API_URL}/api/onboard/completeprivate`,{
            method: "POST", 
            credentials: "include",
            body: formData
        });
	...
	<div className="flex min-h-full flex-col justify-center py-12 sm:px-6 lg:px-8 bg-white dark:bg-slate-600" style={{ backgroundImage: `url(${bgimage})` }}>
		<div className="mt-10 sm:mx-auto sm:w-full sm:max-w-4xl">
			<div className="bg-white px-6 py-12 shadow-sm sm:rounded-lg sm:px-12 dark:bg-gray-800/90 dark:shadow-none dark:outline dark:-outline-offset-1 dark:outline-white/10">
				<form onSubmit={completePrivate}>
					<div className="space-y-12">
						<div className="border-b border-gray-900/10 pb-12 dark:border-white/10">
							<h2 className="text-base/7 font-semibold text-gray-900 dark:text-white">Personal Information</h2>
							<p className="mt-1 text-sm/6 text-gray-600 dark:text-gray-400">
								This information helps others to better know you as a person.
								All information are optional, you can skip it for now.
							</p>
